...
CKPT_PATH = "runs_unet/best.pt"  # <-- change if your best model lives elsewhere
...
# ----------------------------------------------------------------------
# UNet (same architecture as training, but 3 classes)
# ----------------------------------------------------------------------
...
class UNet(nn.Module):
    def __init__(self, in_ch=3, n_classes=3, base=32):
        ...
        self.u4 = nn.ConvTranspose2d(base*16, base*8, 2, stride=2); self.c4 = DoubleConv(base*16, base*8)
...
def load_model(ckpt: str = CKPT_PATH) -> UNet:
    ...
    net = UNet(n_classes=len(CLASSES), base=32).to(DEVICE)
...
if __name__ == "__main__":
    ...
    #)
    run_folder(
        "/Users/senakshikrishnamurthy/Desktop/DS/606 capstone/Capstone-From-2D-Civil-Floor-Drawings-to-3D-Models/data/images/test",
        ckpt=CKPT_PATH,
        out_dir="pred_unet"
    )
