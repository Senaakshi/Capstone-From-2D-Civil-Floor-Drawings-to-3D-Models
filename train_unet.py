...
# -------------------- Config --------------------
CLASSES     = ["background", "wall", "room"]   # start simple; add door/window later
NUM_CLASSES = len(CLASSES)
IMG_SIZE    = 640          # smaller -> less MPS memory (try 768 if you can)
BATCH       = 1            # keep 1 on MPS to avoid OOM
EPOCHS      = 40
LR          = 1e-3
ROOT        = "data"       # expect data/images/{train,val}, data/masks/{train,val}
SAVE_DIR    = "runs_unet"
...
class UNet(nn.Module):
    def __init__(self, in_ch=3, n_classes=2, base=32):  # base=32 to save memory
        super().__init__()
