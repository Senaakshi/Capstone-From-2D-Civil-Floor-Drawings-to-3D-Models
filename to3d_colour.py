...
COLOR_WALL  = [0, 180, 180, 160]   # R,G,B,A
  # light brown, opaque
# Example teal semi-transparent:
# COLOR_WALL = [0, 180, 180, 160]
...
def extrude(poly, height_m, z0=0.0, scale=1.0, color=None):
    """
    Extrude a 2D polygon into a 3D prism and optionally color it.
    `color` should be [R,G,B,A] in 0–255.
    """
...
def build_mesh(json_path, out_path):
    ...
    for rp in iter_polys(room_union):
        meshes.append(
            extrude(rp, FLOOR_T, z0=0.0, scale=PX_TO_M, color=COLOR_FLOOR)
        )

    # walls (colored)
    for wp in iter_polys(wall_union):
        meshes.append(
            extrude(wp, WALL_H, z0=FLOOR_T, scale=PX_TO_M, color=COLOR_WALL)
        )
